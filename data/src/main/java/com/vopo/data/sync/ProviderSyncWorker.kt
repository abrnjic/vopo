...
internal suspend fun reconcileTargetedProviderStatus(
    ...
    provider: com.vopo.data.local.entity.ProviderEntity,
    result: com.vopo.domain.model.Result<Unit>,
    ...
    when (result) {
        ...
        is com.vopo.domain.model.Result.Loading -> Unit
    }
}
    ...
    private suspend fun syncXtreamProviderIfStale(
        ...
        if (liveStale) {
            when (val liveResult = entryPoint.syncManager().retrySection(
                ...
                is com.vopo.domain.model.Result.Error -> return liveResult
                else -> Unit
            }
        }
        if (epgStale) {
            when (val epgResult = entryPoint.syncManager().syncEpg(provider.id, force = false)) {
                is com.vopo.domain.model.Result.Error -> return epgResult
                else -> Unit
            }
        }
        if (movieIndexDue) {

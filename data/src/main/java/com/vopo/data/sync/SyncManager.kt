...
import com.vopo.data.remote.http.buildAppRequestProfile
import com.vopo.data.remote.http.toGenericRequestProfile
import com.vopo.data.remote.stalker.StalkerApiService
import com.vopo.data.remote.stalker.StalkerPlaybackMode
import com.vopo.data.remote.stalker.StalkerProvider
import com.vopo.data.remote.stalker.StalkerProviderProfile
import com.vopo.data.remote.jellyfin.JellyfinProvider
import com.vopo.data.remote.stalker.StalkerTrafficCoordinator
import com.vopo.data.remote.dto.XtreamCategory
import com.vopo.data.remote.dto.XtreamSeriesItem
import com.vopo.data.remote.dto.XtreamStream
import com.vopo.data.remote.xtream.OkHttpXtreamApiService
import com.vopo.data.remote.xtream.XtreamAuthenticationException
import com.vopo.data.remote.xtream.XtreamNetworkException
import com.vopo.data.remote.xtream.XtreamParsingException
import com.vopo.data.remote.xtream.XtreamRequestException
import com.vopo.data.remote.xtream.XtreamResponseTooLargeException
import com.vopo.data.remote.xtream.XtreamApiService
import com.vopo.data.remote.xtream.XtreamProvider
import com.vopo.data.remote.xtream.XtreamUrlFactory
import com.vopo.data.security.CredentialCrypto
import com.vopo.data.util.AdultContentClassifier
import com.vopo.data.util.UrlSecurityPolicy
...
import com.vopo.domain.model.VodSyncMode
...
import com.vopo.domain.sync.Section
import com.vopo.domain.sync.SyncProgress
...
class SyncManager @Inject constructor(
    ...
    private val preferencesRepository: com.vopo.data.preferences.PreferencesRepository,
    ...
    ): com.vopo.domain.model.Result<Unit> {
        val providerEntity = providerDao.getById(providerId)
            ?: return com.vopo.domain.model.Result.error("Provider $providerId not found")
        ...
        return if (providerEntity.type == ProviderType.STALKER_PORTAL) {
            ...
            withProviderLock(providerId) {
                val freshProviderEntity = providerDao.getById(providerId)
                    ?: return@withProviderLock com.vopo.domain.model.Result.error("Provider $providerId not found")
    ...
    private suspend fun syncEpgLocked(
        providerEntity: com.vopo.data.local.entity.ProviderEntity,
        force: Boolean,
        ...
        if (!force && provider.type == ProviderType.STALKER_PORTAL && hasPendingStalkerCatalogIndex(provider.id)) {
            ...
            return com.vopo.domain.model.Result.success(Unit)
        }
        ...
        return try {
            ...
            com.vopo.domain.model.Result.success(Unit)
            ...
            com.vopo.domain.model.Result.error(safeMessage, e)
        }
    }
    ...
    ): com.vopo.domain.model.Result<Unit> = withProviderLock(providerId) lock@{
        try {
            val providerEntity = providerDao.getById(providerId)
                ?: return@lock com.vopo.domain.model.Result.error("Provider $providerId not found")
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.vopo.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
    ...
    private suspend fun syncXtreamIndexFirst(
        ...
        syncProgressBus.emit(
            com.vopo.domain.sync.SyncProgress(
                section = com.vopo.domain.sync.Section.VOD,
                current = 0,
                total = 0,
                currentLabel = "",
        ...
        syncProgressBus.emit(
            com.vopo.domain.sync.SyncProgress(
                section = com.vopo.domain.sync.Section.SERIES,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = liveCount
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock com.vopo.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.vopo.domain.model.Result.success(Unit)
        }
        ...
        if (warnings.isNotEmpty()) {
            ...
            com.vopo.domain.model.Result.error(warnings.first(), exception)
        } else {
            com.vopo.domain.model.Result.success(Unit)
        }
    }
    ...
    private suspend fun processStalkerSummaryIndexSection(
        ...
        data class StalkerCategoryFetchResult(
            ...
            val result: Result<com.vopo.data.remote.stalker.StalkerPagedResult<out Any>>
        )
            ...
            when (val result = fetchStalkerWildcardSummaryPageWithRecovery(api, contentType, wildcardCategory.categoryId, nextPage)) {
                is Result.Success -> {
                    ...
                    val anomaly = detectStalkerPageAnomaly(
                        hydration = currentHydration,
                        requestedPage = nextPage,
                        pagedResult = com.vopo.data.remote.stalker.StalkerPagedResult(
                            items = visibleItems,
    ...
    private suspend fun markStalkerAttemptStarted(
        ...
        when (contentType) {
            ContentType.MOVIE -> movieCategoryHydrationDao.upsert(
                com.vopo.data.local.entity.MovieCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
            ...
            ContentType.SERIES -> seriesCategoryHydrationDao.upsert(
                com.vopo.data.local.entity.SeriesCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
                    lastHydratedAt = hydration?.lastHydratedAt ?: 0L,
                    itemCount = hydration?.itemCount ?: 0,
    ...
    private suspend fun fetchStalkerSummaryPageWithRecovery(
        ...
        categoryId: Long,
        page: Int
    ): Result<com.vopo.data.remote.stalker.StalkerPagedResult<out Any>> {
    ...
    private suspend fun fetchStalkerSummaryFirstPageFallback(
        ...
        contentType: ContentType,
        categoryId: Long
    ): Result<com.vopo.data.remote.stalker.StalkerPagedResult<out Any>> = when (contentType) {
        ContentType.MOVIE -> when (val result = api.getVodStreams(categoryId)) {
            is Result.Success -> Result.success(
                com.vopo.data.remote.stalker.StalkerPagedResult(
                    items = result.data,
                    page = 1,
                    totalPages = 1,
    ...
    private fun detectStalkerPageAnomaly(
        hydration: StalkerHydrationSnapshot?,
        requestedPage: Int,
        pagedResult: com.vopo.data.remote.stalker.StalkerPagedResult<out Any>,
        pageFingerprint: String?
    ): String? {
    ...
    private suspend fun syncStalker(
        ...
        if (force || ContentCachePolicy.shouldRefresh(metadata.lastMovieSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            ...
            val categories = when (val categoriesResult = api.getVodCategories()) {
                is com.vopo.domain.model.Result.Success -> categoriesResult.data
                is com.vopo.domain.model.Result.Error -> {
                ...
                is com.vopo.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
            }
            emitCatalogSyncProgress(
                section = Section.VOD,
    ...
    private suspend fun syncJellyfin(
        ...
        try {
            ...
            if (resolvedMovies is com.vopo.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Movies error: ${resolvedMovies.message}")
            if (resolvedSeries is com.vopo.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Series error: ${resolvedSeries.message}")

            if (resolvedMovies is com.vopo.domain.model.Result.Error && resolvedSeries is com.vopo.domain.model.Result.Error) {
            ...
            val movieEntities = (resolvedMovies as? com.vopo.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? com.vopo.domain.model.Result.Success)?.data.orEmpty()
            ...
            if (movieEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.vopo.domain.model.ContentType.MOVIE.name, listOf(
                        com.vopo.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 1L, name = "Movies",
                            type = com.vopo.domain.model.ContentType.MOVIE
                        )
                    ))
                }
            }
            ...
            if (seriesEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.vopo.domain.model.ContentType.SERIES.name, listOf(
                        com.vopo.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 2L, name = "Series",
                            type = com.vopo.domain.model.ContentType.SERIES
                        )
                    ))
                }
            }
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<com.vopo.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        runCatching {
            ...
            api.streamBulkEpg(periodHours = 6) { program ->
                ...
                if (result is com.vopo.domain.model.Result.Error) {
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            runCatching {
                ...
                if (streamResult is com.vopo.domain.model.Result.Error) {
    ...
    private suspend fun syncXtreamLiveCatalog(
        ...
        syncProgressBus.emit(
            com.vopo.domain.sync.SyncProgress(
                section = com.vopo.domain.sync.Section.LIVE,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = 0
    ...
    private suspend fun loadStalkerChannelsByCategory(
        api: StalkerProvider,
        categories: List<com.vopo.domain.model.Category>,
        onProgress: ((String) -> Unit)?
    ...
    private suspend fun syncStalkerLiveCatalogStaged(
        ...
        try {
            ...
            when (val streamResult = api.streamLiveStreams { channel ->
                ...
                is com.vopo.domain.model.Result.Success -> {
                    flushBatch()
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is com.vopo.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: com.vopo.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is com.vopo.domain.model.Result.Success -> result.data
            is com.vopo.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is com.vopo.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }

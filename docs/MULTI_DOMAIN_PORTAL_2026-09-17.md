...
Nastavak provjere: javne datoteke potvrđuju aktivnu klijentsku izmjenu iz runtime popravka, ali /api/domains i dalje vraća HTML 500. Lokalno je reproduciran ERR_REQUIRE_ESM s Node.js 24 i --no-experimental-require-module. Pripremljeni dodatni popravak u next.config.ts bundla firebase-admin, jwks-rsa i jose preko transpilePackages. Build prolazi i s isključenim require(ESM); stvarni lokalni produkcijski server vraća JSON 401 za domene, admin/users, admin/resellers i reseller/activate umjesto pada prije handlera. Dodan je npm run test:runtime i CI provjera tog startup slučaja.

Produkcijska potvrda 2026-09-18: nakon korisničkog nastavka pristup Vercelu radi. Commit c697dfe49549c8c17e1d12f91917f066daf02de8 objavljen je kroz deployment 5SU5NRhiSou6ZfYBSHwkRTB32ZZh, Ready (1m 11s), dodijeljen www.vopoapp.com. Javni GET /api/domains?catalog=1 bez tokena vraća HTTP 401 application/json {"error":"No token provided"}, umjesto prethodnog HTML 500.

U stvarno prijavljenom administratorskom panelu (admin@vopoapp.com) kartica Domene uspješno učitava prazan zajednički katalog i omogućuje unos adrese. Odabirom postojećeg resellera potvrđen je dohvat njegove postojeće domene te dostupni Uredi/Ukloni; Uredi otvara popunjeno polje i aktivan Spremi. Uređivanje je otkazano bez promjene produkcijskih podataka. Nove domene nisu izmišljene niti dodane radi testiranja. Lokalni dokaz builda: /private/tmp/vopo-domains-bundled-build.log. API CRUD je pokriven ranije navedenim automatiziranim testovima; stvarno emitiranje s dvije domene i dalje nije potvrđeno.

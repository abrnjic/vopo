...
class PluginMessengerClient @Inject constructor(
    ...
    suspend fun send(
        ...
        val replyMessenger = Messenger(Handler(Looper.getMainLooper()) { message ->
            ...
            if (response.getString(VopoPluginContract.KEY_REQUEST_ID) == requestId &&
        ...
        try {
            withContext(Dispatchers.Main.immediate) {
                val intent = Intent(VopoPluginContract.ACTION_PLUGIN_SERVICE).apply {
            ...
            val request = Message.obtain(null, what).apply {
                ...
                this.data = Bundle(data).apply {
                    putInt(VopoPluginContract.KEY_API_VERSION, VopoPluginContract.API_VERSION)
                    putString(VopoPluginContract.KEY_REQUEST_ID, requestId)
                }
            }
            try {

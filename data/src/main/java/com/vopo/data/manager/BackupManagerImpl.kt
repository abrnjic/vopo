...
import com.vopo.data.preferences.ParentalPinBackupData
import com.vopo.data.preferences.PreferencesRepository
import com.vopo.data.security.CredentialCrypto
import com.vopo.domain.manager.BackupData
import com.vopo.domain.manager.BackupConflictStrategy
import com.vopo.domain.manager.BackupImportPlan
import com.vopo.domain.manager.BackupImportResult
import com.vopo.domain.manager.BackupManager
import com.vopo.domain.manager.BackupPreview
import com.vopo.domain.manager.RecordingScheduleImportDisposition
import com.vopo.domain.manager.RecordingScheduleImportOutcome
import com.vopo.domain.manager.RecordingScheduleImportSummary
import com.vopo.domain.manager.ProtectedCategoryBackup
import com.vopo.domain.manager.RecordingManager
import com.vopo.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): com.vopo.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext com.vopo.domain.model.Result.error("Failed to open output stream")

            com.vopo.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            com.vopo.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): com.vopo.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext com.vopo.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext com.vopo.domain.model.Result.error("Unsupported backup version")
            }
            if (backupData.isStructurallyEmpty()) {
                return@withContext com.vopo.domain.model.Result.error("Backup file does not contain any importable data")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext com.vopo.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            com.vopo.domain.model.Result.success(
                BackupImportResult(
            ...
            com.vopo.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                com.vopo.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        prefs["playerDecoderMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val decoderMode = com.vopo.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = com.vopo.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = com.vopo.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        prefs["playerLiveStreamFormatMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val formatMode = com.vopo.domain.model.LiveStreamFormatMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (formatMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = com.vopo.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<com.vopo.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.vopo.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<com.vopo.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.vopo.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun com.vopo.domain.model.RecordingItem.toScheduledRecordingBackup(
    provider: com.vopo.domain.model.Provider,
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<com.vopo.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun com.vopo.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<com.vopo.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<com.vopo.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<com.vopo.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<com.vopo.domain.model.PlaybackHistory>>() {}.type

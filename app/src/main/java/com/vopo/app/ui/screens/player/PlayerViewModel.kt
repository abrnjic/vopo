...
import com.vopo.app.cast.CastConnectionState
import com.vopo.app.cast.CastManager
import com.vopo.app.cast.CastMediaRequest
import com.vopo.app.cast.CastStartResult
import com.vopo.app.di.MainPlayerEngine
import com.vopo.app.player.LivePreviewHandoffManager
import com.vopo.app.player.LiveTranslationSession
...
import com.vopo.data.remote.stalker.StalkerUrlFactory
...
import com.vopo.domain.model.CombinedM3uProfileMember
import com.vopo.domain.model.Program
import com.vopo.domain.model.ContentType
import com.vopo.domain.model.DecoderMode
import com.vopo.domain.model.Episode
import com.vopo.domain.model.Favorite
import com.vopo.domain.model.LiveChannelObservedQuality
...
import com.vopo.domain.model.RecordingRequest
...
import com.vopo.domain.model.VideoFormat
...
import com.vopo.domain.usecase.ScheduleRecordingCommand
...
import com.vopo.player.AUDIO_VIDEO_OFFSET_MAX_MS
import com.vopo.player.AUDIO_VIDEO_OFFSET_MIN_MS
import com.vopo.player.PlaybackState
import com.vopo.player.PlayerEngine
import com.vopo.player.PlayerError
import com.vopo.player.PlayerSubtitleStyle
import com.vopo.player.timeshift.LiveTimeshiftBackend
import com.vopo.player.timeshift.LiveTimeshiftState
import com.vopo.player.timeshift.LiveTimeshiftStatus
import com.vopo.player.timeshift.TimeshiftConfig
import com.vopo.player.playback.applyUnsafeTlsBypass
...
class PlayerViewModel @Inject constructor(
    ...
    internal val favoriteRepository: com.vopo.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    internal val providerRepository: com.vopo.domain.repository.ProviderRepository,
    internal val combinedM3uRepository: CombinedM3uRepository,
    internal val preferencesRepository: com.vopo.data.preferences.PreferencesRepository,
    ...
    internal val pluginManager: VopoPluginManager,
    ...
    internal val currentChannelFlow = MutableStateFlow<com.vopo.domain.model.Channel?>(null)
    val currentChannel: StateFlow<com.vopo.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    internal val currentChannelFlowList = MutableStateFlow<List<com.vopo.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<com.vopo.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<com.vopo.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<com.vopo.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
    ...
    val remoteShortcutPreferences = preferencesRepository.remoteShortcutPreferences
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000L), com.vopo.domain.model.RemoteShortcutPreferences())
    ...
    private var preferredSurfaceMode: com.vopo.domain.model.PlayerSurfaceMode =
        com.vopo.domain.model.PlayerSurfaceMode.AUTO
     ...
     */
    internal var channelList: List<com.vopo.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, com.vopo.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun logRepositoryFailure(operation: String, result: com.vopo.domain.model.Result<Unit>) {
    ...
    val playerStats: StateFlow<com.vopo.player.PlayerStats> =
        activeEngineState(com.vopo.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<com.vopo.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<com.vopo.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<com.vopo.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    internal suspend fun preparePlayer(
        streamInfo: com.vopo.domain.model.StreamInfo,
        requestVersion: Long,
    ...
    private suspend fun probePlaybackUrl(streamInfo: com.vopo.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private suspend fun shouldProbePlaybackUrl(url: String): Boolean {
        ...
        if (provider.type != com.vopo.domain.model.ProviderType.STALKER_PORTAL && cacheKey in probePassedPlaybackKeys) {
            return false
        }
        return (
            provider.type == com.vopo.domain.model.ProviderType.XTREAM_CODES ||
                provider.type == com.vopo.domain.model.ProviderType.STALKER_PORTAL
            ) &&
                    ...
                    _playerDiagnostics.update {
                        it.copy(
                            ...
                            providerSourceLabel = when (provider.type) {
                                com.vopo.domain.model.ProviderType.XTREAM_CODES -> "Xtream Codes"
                                com.vopo.domain.model.ProviderType.M3U -> "M3U Playlist"
                                com.vopo.domain.model.ProviderType.STALKER_PORTAL -> "Stalker/MAG Portal"
                                com.vopo.domain.model.ProviderType.JELLYFIN -> "Jellyfin"
                            }
                        )
                    }
                }
    ...
    internal fun updateChannelDiagnostics(channel: com.vopo.domain.model.Channel) {
    ...
    internal suspend fun resolvePlaybackStreamInfo(
        ...
        providerId: Long,
        contentType: ContentType
    ): com.vopo.domain.model.StreamInfo? {

...
data class VopoPluginManifest(
    ...
    val supportsConfigurationActivity: Boolean
        get() = hasCapability(VopoPluginContract.CAPABILITY_CONFIGURATION_ACTIVITY) &&
            !configurationActivityAction.isNullOrBlank()

    val usesActivityConfiguration: Boolean
        get() = configurationMode == VopoPluginContract.CONFIGURATION_MODE_ACTIVITY

    val supportsHostRenderedConfiguration: Boolean
        get() = configurationMode == VopoPluginContract.CONFIGURATION_MODE_HOST_SCHEMA ||
            (configurationMode != VopoPluginContract.CONFIGURATION_MODE_ACTIVITY &&
                hasCapability(VopoPluginContract.CAPABILITY_CONFIGURATION_SCHEMA))

    val canConfigure: Boolean
        get() = supportsHostRenderedConfiguration || supportsConfigurationActivity
}

data class InstalledVopoPlugin(
    val packageName: String,
    val serviceClassName: String,
    val appLabel: String,
    val manifest: VopoPluginManifest,
    val enabled: Boolean,
...
data class PluginConfigurationSnapshot(
    val plugin: InstalledVopoPlugin,

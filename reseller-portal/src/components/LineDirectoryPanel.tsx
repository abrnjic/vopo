...
type Line = { id: string; deviceId?: string; resellerId?: string; ownerAccount?: Account | null; creatorAccount?: Account | null; customerName?: string; customerContact?: string; selectedDomain?: string; xtreamConfig?: { username?: string }; hasLinePassword?: boolean; hasLinePin?: boolean; status?: string; isLifetime?: boolean; expiresAt?: string };
...
const accountLabel = (account?: Account | null) => account
  ? `${account.role === 'subseller' ? 'Subseller' : account.role === 'admin' ? 'Admin' : account.role === 'reseller' ? 'Reseller' : 'Račun'}: ${account.label}${account.parentLabel ? ` (glavni: ${account.parentLabel})` : ''}`
  : 'Nije zabilježeno';
...
export default function LineDirectoryPanel({ initialEditId, onLineUpdated }: { initialEditId?: string | null; onLineUpdated?: (line: Line) => void }) {
  ...
  const visible = lines.filter(line => [line.deviceId, line.customerName, line.customerContact, line.selectedDomain, line.ownerAccount?.label, line.creatorAccount?.label].some(value => String(value || '').toLowerCase().includes(search.toLowerCase())));
  ...
  return <div className="space-y-5">
    ...
    {detail && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"><div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-gray-700 bg-gray-900 p-6 shadow-2xl">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">{[['Status', detail.line.status], ['Licenca', detail.line.isLifetime ? 'Trajna' : 'Vremenska'], ['Ističe', detail.line.isLifetime ? 'Nikada' : detail.line.expiresAt ? new Date(detail.line.expiresAt).toLocaleString('hr-HR') : '—'], ...(userData?.role === 'admin' ? [['Vlasnik računa', detail.line.ownerAccount ? accountLabel(detail.line.ownerAccount) : detail.line.resellerId === 'self_registered' ? 'Nije dodijeljeno' : 'Nije zabilježeno'], ['Liniju kreirao', accountLabel(detail.line.creatorAccount)]] : []), ['Kupac', detail.line.customerName || 'Nije upisan']].map(([label, value]) => <div key={label} className="rounded-xl bg-gray-800 p-3"><p className="text-gray-500">{label}</p><p className="mt-1 break-words font-semibold text-white">{value || '—'}</p></div>)}</div>

...
object BackupFileBridge {
    ...
    fun createExportFile(baseDir: File): File {
        ...
        return File(backupsDir, "vopo_backup_${LocalDateTime.now().format(exportNameFormatter)}.json")
            .also { file ->
    ...
    fun copyToImportInbox(context: Context, sourceUri: Uri): Uri? {
        ...
        val targetFile = File(inboxDir, "vopo_import_${System.currentTimeMillis()}.json")
        return runCatching {
    ...
    fun buildShareIntent(uri: Uri): Intent {
        ...
        return Intent.createChooser(sendIntent, "Share Vopo backup")
            .addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
    }

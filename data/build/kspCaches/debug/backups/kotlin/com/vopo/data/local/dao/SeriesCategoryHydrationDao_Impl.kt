...
public class SeriesCategoryHydrationDao_Impl(
  __db: RoomDatabase,
) : SeriesCategoryHydrationDao {
  private val __db: RoomDatabase

  private val __insertAdapterOfSeriesCategoryHydrationEntity:
      EntityInsertAdapter<SeriesCategoryHydrationEntity>
  init {
    this.__db = __db
    this.__insertAdapterOfSeriesCategoryHydrationEntity = object : EntityInsertAdapter<SeriesCategoryHydrationEntity>() {
      protected override fun createQuery(): String = "INSERT OR REPLACE INTO `series_category_hydration` (`provider_id`,`category_id`,`last_hydrated_at`,`item_count`,`last_status`,`last_error`,`last_loaded_page`,`last_attempted_page`,`last_successful_page`,`total_pages`,`is_complete`,`page_size`,`retry_after_ms`,`failure_count`,`retry_budget_remaining`,`last_page_fingerprint`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"

      protected override fun bind(statement: SQLiteStatement, entity: SeriesCategoryHydrationEntity) {
  ...
  public override suspend fun upsert(metadata: SeriesCategoryHydrationEntity): Unit = performSuspending(__db, false, true) { _connection ->
    __insertAdapterOfSeriesCategoryHydrationEntity.insert(_connection, metadata)
  }

  public override suspend fun `get`(providerId: Long, categoryId: Long): SeriesCategoryHydrationEntity? {
    val _sql: String = "SELECT * FROM series_category_hydration WHERE provider_id = ? AND category_id = ?"
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        val _result: SeriesCategoryHydrationEntity?
        if (_stmt.step()) {
  ...
  public override suspend fun delete(providerId: Long, categoryId: Long) {
    val _sql: String = "DELETE FROM series_category_hydration WHERE provider_id = ? AND category_id = ?"
  ...
  public override suspend fun deleteByProvider(providerId: Long) {
    val _sql: String = "DELETE FROM series_category_hydration WHERE provider_id = ?"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1

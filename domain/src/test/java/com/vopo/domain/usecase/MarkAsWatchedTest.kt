...
class MarkAsWatchedTest {
    ...
    private class FakePlaybackHistoryRepository : PlaybackHistoryRepository {
        ...
        override suspend fun markAsWatched(history: PlaybackHistory): com.vopo.domain.model.Result<Unit> {
            lastMarkedHistory = history
            return com.vopo.domain.model.Result.success(Unit)
        }
        override suspend fun recordPlayback(history: PlaybackHistory) = com.vopo.domain.model.Result.success(Unit)
        override suspend fun updateResumePosition(history: PlaybackHistory) = com.vopo.domain.model.Result.success(Unit)
        override suspend fun flushPendingProgress() = com.vopo.domain.model.Result.success(Unit)
        override suspend fun removeFromHistory(contentId: Long, contentType: ContentType, providerId: Long) = com.vopo.domain.model.Result.success(Unit)
        override suspend fun clearAllHistory() = com.vopo.domain.model.Result.success(Unit)
        override suspend fun clearHistoryForProvider(providerId: Long) = com.vopo.domain.model.Result.success(Unit)
        override suspend fun clearLiveHistoryForProvider(providerId: Long) = com.vopo.domain.model.Result.success(Unit)
    }
}

...
import com.vopo.data.remote.dto.XtreamSeason
import com.vopo.data.remote.dto.XtreamSeriesInfoResponse
import com.vopo.data.remote.stalker.StalkerCategoryRecord
import com.vopo.data.remote.stalker.StalkerEpisodeRecord
...
import com.vopo.data.remote.stalker.StalkerSeasonRecord
import com.vopo.data.remote.stalker.StalkerSeriesDetails
...
import com.vopo.domain.model.VodDuplicateConfidence
import com.vopo.domain.model.VodDuplicateHandlingMode
import com.vopo.domain.model.VodSeriesVariant
import com.vopo.domain.model.VodVariantObservation
...
class SeriesRepositoryImplTest {
    ...
    fun `stalker series preview loads only first page`() = runTest {
        ...
        whenever(categoryDao.getByProviderAndType(7L, ContentType.SERIES.name)).thenReturn(
            flowOf(listOf(com.vopo.data.local.entity.CategoryEntity(providerId = 7L, categoryId = 77L, name = "Drama", type = ContentType.SERIES)))
        )
        whenever(providerDao.getById(7L)).thenReturn(
            ProviderEntity(
                id = 7L,
    ...
    fun `getSeriesDetails falls back to remote series id lookup`() = runTest {
        ...
        assertThat(result).isInstanceOf(com.vopo.domain.model.Result.Success::class.java)
        val series = (result as com.vopo.domain.model.Result.Success).data
